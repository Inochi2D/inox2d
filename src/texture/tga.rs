//! Port of imagefmt's TGA decoder written in D.
//!
//! See https://github.com/tjhann/imagefmt
...
pub(crate) fn read_tga_header<R: Read + Seek>(reader: &mut R) -> Result<TgaHeader, TgaDecodeError> {
    ...
    let data_type = read_u8(reader)?
        .try_into()
        .map_err(|_| TgaDecodeError::InvalidHeader)?;
        ...
        for _ in 0..header.height {
            ...
            while wanted > 0 {
                if packet_len == 0 {
                    ...
                    packet_len =
                        ((packet_head & TGA_FLAG_PACKET_LEN) + 1) as usize * channels as usize;
                }
