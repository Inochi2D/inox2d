        ...
        for binding in &self.bindings {
            ...
            match binding.values {
                ...
                BindingValues::TransformTX(ref matrix) => {
                    let (out_top, out_bottom) =
                        ranges_out(matrix, x_mindex, x_maxdex, y_mindex, y_maxdex);
...
impl Puppet {
    ...
    pub fn set_named_param(&mut self, param_name: &str, val: Vec2) {
        let param_uuid = (self.param_names.get(param_name))
    ...
    pub fn set_param(&mut self, param_uuid: ParamUuid, val: Vec2) {
        let param = (self.params.get_mut(&param_uuid))
            .unwrap_or_else(|| panic!("No parameter with uuid: {:?}", param_uuid));
